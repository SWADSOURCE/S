...
async def hi(event):
    await event.reply(
        "**[ ᥉᥆υᖇᥴᥱ 𝙰𝚅𝙰𝚃𝙰𝚁 ](https://t.me/source_av)**\n✦┅━╍━╍╍━━╍━━╍━┅✦\n\n• اوامر التحكم تتيح لمستخدم اخر ان يستخدم اوامر تنصيبك وهو غير منصب بوت افاتار حيث سيصبح متحكم في اوامر تنصيبك يرجى الانتباه من انه سيحصل على صلاحيات الاوامر وقد يسبب خطورة لك اذا كنت لا تثق في المستخدم الذي اضفته. \n\n\n`.التحكم` تفعيل/تعطيل\n• تستخدم هذه الميزة لتفعيل/لتعطيل خاصية التحكم بأوامر تنصيبك للمستخدمين الذين أضفتهم في قائمة المتحكمين\n\n`.المتحكمين`\n• لعرض المستخدمين الذين اضفتهم الى قائمة المستخدمين المتحكمين\n\n`.اضف متحكم`\n• بالرد على المستخدم لأضافته متحكم في تنصيب بوت افاتار الخاص بك\n\n`.ازالة متحكم`\n• بالرد على المستخدم لازالته من قائمة المتحكمين"
    ) 
    await event.delete()
...
@sbb_b.ar_cmd(pattern="اوامر الرفع")
async def hi(event):
    await event.reply(
        "⌯╼═══❬ ᥉᥆υᖇᥴᥱ 𝙰𝚅𝙰𝚃𝙰𝚁 ❭═══╾⌯\n\n❃ `.رفع بقلبي`\n\n❃ `.تنزيل بقلبي`\n\n❃ `.رفع خول`\n\n❃ `.تنزيل خول`\n\n❃ `.رفع سيمب`\n\n❃ `.تنزيل سيمب`\n\n❃ `.رفع شاذ`\n\n❃ `.تنزيل شاذ`\n\n❃ `.رفع عرص`\n\n❃ `.تنزيل عرص`\n\n❃ `.رفع متوحد`\n\n❃ `.تنزيل متوحد`\n\n❃ `.رفع حمار`\n\n❃ `.تنزيل حمار`\n\n⌯╼═══❬ ᥉᥆υᖇᥴᥱ 𝙰𝚅𝙰𝚃𝙰𝚁 ❭═══╾⌯"
    ) 
